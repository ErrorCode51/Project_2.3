...
public class OthelloRules implements Rules {
    ...
    public boolean testForLegal(Board board, Stone stone, byte row, byte column, int counter, boolean turnOver) {
        ...
        byte x = stone.getX();
        byte y = stone.getY();
        char colour = stone.getIdentifier();
        // Temporary fix
        if (row == 0 && column == 0) {
        ...
        if (board.get((byte) (x + row), (byte) (y + column)).equals(colour)) {
    ...
    public void findAllLegal(Board board) {
        for (byte row = 0; row < board.getSize(); row++) {
            for (byte column = 0; column < board.getSize(); column++) {
                Stone stoned = new OthelloStone(row, column, 'B');
                if (testForLegal(board, stoned, false)) {
                    legalBlack.add(stoned);
                } else {
                    stoned.turnOver();
                    if (testForLegal(board, stoned, false)) {
                        legalWhite.add(stoned);
                    }
                }
            }
        }
