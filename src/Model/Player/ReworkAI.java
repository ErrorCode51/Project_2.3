...
public class ReworkAI implements Player {
    ...
    public ReworkAI(char identifier) {
    ...
    public byte[] placeStone(Board board, Rules rules) {
        ...
        System.out.println("Sekiro: " + branches.get(indexBestPlacement));
    ...
    private void findBestPlacement(final Node root) {
        ...
        for (Node branch : branches) {
            ...
            branch.setScore(score((OthelloStone) branch.getStone(), branch.getBoard()));
            // System.out.println(branch.getStone() + ", " + branch.getScore());
            if (branch.getStone().getIdentifier() == identifier) {
                byte score = 1;
                score *= branch.getBoard().getScoreByIdentifier(branch.getStone().getIdentifier());
                if (score > branch.getAlpha()) {
                    branch.setAlpha(score);
                    if (branch.getPrevious() != null) {
                        if (score > branch.getPrevious().getAlpha()) {
                ...
                byte score = -1;
                score *= branch.getBoard().getScoreByIdentifier(branch.getStone().getIdentifier());
                if (score < branch.getBeta()) {
                    branch.setBeta(score);
                    if (branch.getPrevious() != null) {
                        if (score < branch.getPrevious().getBeta()) {
...
//        }
    ...
    private byte score(OthelloStone stone, OthelloBoard board) {
        byte score = 0;
        // Scoring boardstate
        if (board.gameOver(board) == identifier) {
            score += 30;
        }
        // Scoring the corners
